...
from training.optimizer.adan_sf import adan_sf
...
class Trainer:
    def __init__(self, config: TrainingConfig) -> None:
        ...
        self.best_psnr = 0.0
        self.best_ssim = 0.0
        self.pretrained_conv1: Optional[torch.Tensor] = None
    ...
    def _load_pretrained(self, path: str) -> None:
        try:
            ...
            for _, (k, v) in enumerate(list(state_dict.items())[:5]):
    ...
    def _normalize_input(self, x: torch.Tensor) -> torch.Tensor:
        return x * NORMALIZATION_FACTOR - NORMALIZATION_OFFSET

    def _denormalize_output(self, x: torch.Tensor) -> torch.Tensor:
        return (x + NORMALIZATION_OFFSET) / NORMALIZATION_FACTOR
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, hr_batch in enumerate(self.train_loader):
            ...
            hr_patch = hr_batch.to(self.device)

            with torch.no_grad():
                if batch_idx == 0:
                    self._log_input_stats(hr_patch)

                lr_patch, hr_original = self.train_loader.dataset.transform.degrade_pair(hr_patch)

                if batch_idx == 0:
                    self._log_degradation_stats(lr_patch, hr_original)

                lr_patch = self._normalize_input(lr_patch).to(self.device)

                if batch_idx == 0:
                    self._log_normalized_stats(lr_patch)

            self.optimizer.zero_grad()
            sr_patch = self.model(lr_patch)

            if batch_idx == 0:
                self._log_model_output(sr_patch)

            sr_for_loss = torch.clamp(
                self._denormalize_output(sr_patch),
                MIN_PIXEL_VALUE,
                MAX_PIXEL_VALUE
            )

            if batch_idx == 0:
                self._log_loss_input(sr_for_loss)

            loss, losses = self.loss_composer(sr_for_loss, hr_original)

            if batch_idx == 0:
                self._log_losses(loss, losses)
            ...
            self._update_progress(batch_idx, loss.item(), losses, global_step)
    ...
    def _log_input_stats(self, hr_patch: torch.Tensor) -> None:
    ...
    def _log_degradation_stats(self, lr_patch: torch.Tensor, hr_original: torch.Tensor) -> None:
        print("\n  After degradation:")
    ...
    def _log_normalized_stats(self, lr_patch: torch.Tensor) -> None:
        print("\n  LR after normalization to [-1,1]:")
        print(f"  LR range: [{lr_patch.min():.3f}, {lr_patch.max():.3f}]")
        print(f"  LR mean: {lr_patch.mean():.3f}")

    def _log_model_output(self, sr_patch: torch.Tensor) -> None:
        print("\n🤖 MODEL OUTPUT:")
    ...
    def _log_loss_input(self, sr_for_loss: torch.Tensor) -> None:
        print("\n  SR after conversion to [0,1] for loss:")
        print(f"  SR range: [{sr_for_loss.min():.3f}, {sr_for_loss.max():.3f}]")
        print(f"  SR mean: {sr_for_loss.mean():.3f}")

    def _log_losses(self, loss: torch.Tensor, losses: Dict[str, float]) -> None:
        print("\n📉 LOSSES:")
    ...
    def _log_weight_changes(self) -> None:
        ...
        if self.pretrained_conv1 is not None:
            ...
            if change > WEIGHT_CHANGE_THRESHOLD:
                print("  ⚠️  Large weight change detected! LR might be too high.")

    def _update_progress(self, batch_idx: int, loss: float, losses: Dict[str, float], global_step: int) -> None:
    ...
    def validate(self) -> Dict[str, float]:
        ...
        for hr_batch in self.val_loader:
            hr_patch = hr_batch.to(self.device)
            ...
            lr_patch = self._normalize_input(lr_patch)
            sr_patch = self.model(lr_patch)
            sr_patch = torch.clamp(
                self._denormalize_output(sr_patch),
                MIN_PIXEL_VALUE,
                MAX_PIXEL_VALUE
            )
